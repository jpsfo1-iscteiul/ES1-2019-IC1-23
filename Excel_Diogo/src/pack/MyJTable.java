...
public class MyJTable {
	...
	private Object[][] getValues(){
		...
		for (int i = 0; i < myFile.getRowList().size(); i++){
			...
			if(row.getIPlasma() != row.getPmd())
				System.out.println("diferente" + row.getMethodID());
		}
		return matrix;
	}
...
//			public void run() {
//				try {
//					MyJTable window = new MyJTable(new MyFile("long-method.xlsx"));
...
//	}
	...
	private void initialize() {
		...
		scrollPane.setBounds(0, 0, 1276, 673);
		frame.getContentPane().add(scrollPane);

		DefaultTableModel dtm = new DefaultTableModel(getValues(), new String[] {
				"MethodID","package","class", "method", "LOC", "CYCLO", "ATFD", "LAA", "is_long_method", "iPlasma", "PMD", "is_feature_envy"
		});
